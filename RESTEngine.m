...
//    NSString *tileDirectory = [documentdir stringByAppendingPathComponent:@"Modules.json"];
//    
  //  [self initWithLoginName:loginName password:password];
    [self.networkQueue setShouldCancelAllRequestsOnFailure:NO];
    ...
    [request setDelegate:self];
   // [request setDownloadDestinationPath:tileDirectory];
	[request setDidFinishSelector:@selector(menuFetchDone:)];
	[request setDidFailSelector:@selector(menuFetchFailed:)];	
...
-(void) fetchWrongMenu
{
    ...
    NSDictionary *mockResponseDictionary = [jsonString mutableObjectFromJSONString];
    NSMutableDictionary *errorDictionary = [[mockResponseDictionary objectForKey:@"error"] mutableCopy];

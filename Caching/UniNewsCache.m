...
#define FileName @"UniNewsItems.archive"
#define kMenuStaleSeconds 10

@implementation UniNewsCache
...
+(void) initialize
{
    NSString *cacheDirectory = [UniNewsCache cacheDirectory];
    ...
    double currentAppVersion = [[UniNewsCache appVersion] doubleValue];
...
+(void) saveMemoryCacheToDisk:(NSNotification *)notification
{
    ...
    {
        NSString *archivePath = [[UniNewsCache cacheDirectory] stringByAppendingPathComponent:filename];  
...
+(void) clearCache
{
    NSArray *cachedItems = [[NSFileManager defaultManager] contentsOfDirectoryAtPath:[UniNewsCache cacheDirectory] 
                                                                               error:nil];

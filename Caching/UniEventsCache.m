...
#define FileName @"UniEventsItems.archive"
#define kMenuStaleSeconds 10

@implementation UniEventsCache
...
+(void) initialize
{
    NSString *cacheDirectory = [UniEventsCache cacheDirectory];
    ...
    double currentAppVersion = [[UniEventsCache appVersion] doubleValue];
...
+(void) saveMemoryCacheToDisk:(NSNotification *)notification
{
    ...
    {
        NSString *archivePath = [[UniEventsCache cacheDirectory] stringByAppendingPathComponent:filename];  
...
+(void) clearCache
{
    NSArray *cachedItems = [[NSFileManager defaultManager] contentsOfDirectoryAtPath:[UniEventsCache cacheDirectory] 
                                                                               error:nil];
...
+(NSString*) cacheDirectory
{
	...
	return [cachesDirectory stringByAppendingPathComponent:@"UniEventsCache"];  
}

...
#define FileName @"SchoolEventsItems.archive"
#define kMenuStaleSeconds 10

@implementation SchoolEventsCache
...
+(void) initialize
{
    NSString *cacheDirectory = [SchoolEventsCache cacheDirectory];
...
+(void) saveMemoryCacheToDisk:(NSNotification *)notification
{
    ...
    {
        NSString *archivePath = [[SchoolEventsCache cacheDirectory] stringByAppendingPathComponent:filename];  
...
+(void) clearCache
{
    NSArray *cachedItems = [[NSFileManager defaultManager] contentsOfDirectoryAtPath:[SchoolEventsCache cacheDirectory] 
                                                                               error:nil];
...
+(NSString*) cacheDirectory
{
	...
	return [cachesDirectory stringByAppendingPathComponent:@"SchoolEventsCache"];  
}

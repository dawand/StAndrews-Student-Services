...
+(void) initialize
{
  NSString *cacheDirectory = [ModulesCache cacheDirectory];
  ...
  double currentAppVersion = [[ModulesCache appVersion] doubleValue];
...
+(void) saveMemoryCacheToDisk:(NSNotification *)notification
{
  ...
  {
    NSString *archivePath = [[ModulesCache cacheDirectory] stringByAppendingPathComponent:filename];  
...
+(void) clearCache
{
  NSArray *cachedItems = [[NSFileManager defaultManager] contentsOfDirectoryAtPath:[ModulesCache cacheDirectory] 
                                                      error:nil];
...
+(void) cacheMenuItems:(NSMutableArray*) menuItems
{
  [self cacheData:[NSKeyedArchiver archivedDataWithRootObject:menuItems]
           toFile:@"Modules.archive"];  
}

+(NSMutableArray*) getCachedMenuItems
{
  return [NSKeyedUnarchiver unarchiveObjectWithData:[self dataForFile:@"Modules.archive"]];
}
...
+(BOOL) isMenuItemsStale
{
  ...
  if([recentlyAccessedKeys containsObject:@"MenuItems.archive"])
	...
	NSString *archivePath = [[ModulesCache cacheDirectory] stringByAppendingPathComponent:@"Modules.archive"];  

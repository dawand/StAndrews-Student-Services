...
-(void)fetchRss
{
    ...
    //new arrivals
    else if([sourceChoice isEqualToString:@"arrivals"]&& [segmentedControlChoice isEqualToString:@"Books"]) {
        xmlData = [[NSMutableData alloc] initWithContentsOfURL:[NSURL URLWithString: NewArrivalsBooks] ];
    }
    else if([sourceChoice isEqualToString:@"arrivals"]&& [segmentedControlChoice isEqualToString:@"DVDs"]) {
        xmlData = [[NSMutableData alloc] initWithContentsOfURL:[NSURL URLWithString: NewArrivalsDVDs] ];
    }
    else if([sourceChoice isEqualToString:@"arrivals"]&& [segmentedControlChoice isEqualToString:@"eBooks"]) {
        xmlData = [[NSMutableData alloc] initWithContentsOfURL:[NSURL URLWithString: NewArrivalsEBooks] ];
    }
    else if([sourceChoice isEqualToString:@"arrivals"]&& [segmentedControlChoice isEqualToString:@"eJournals"]) {
        xmlData = [[NSMutableData alloc] initWithContentsOfURL:[NSURL URLWithString: NewArrivalsEJournals] ];
    }
...
-(NSDictionary*)getItemFromXmlElement:(GDataXMLElement*)xmlItem
{
    ...
    return [NSDictionary dictionaryWithObjectsAndKeys:
        ...
        //    [[[xmlItem elementsForName:@"url"] objectAtIndex:0]stringValue],@"url",
            nil];
}
